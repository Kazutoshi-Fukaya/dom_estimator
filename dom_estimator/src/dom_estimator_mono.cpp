...
DomEstimatorMono::DomEstimatorMono():
    private_nh_("~"),
    // database_(new Database()),
    database_(new SimpleDatabase()),
    ...
    time_count_(0)
{
    ...
    object_map_pub_ = nh_.advertise<multi_localizer_msgs::SimpleObjectMap>("object_map",1);

    ops_with_id_sub_ = nh_.subscribe("ops_with_id",1,&DomEstimatorMono::ops_with_id_callback,this);
}
...
void DomEstimatorMono::ops_with_id_callback(const object_identifier_msgs::ObjectPositionsWithIDConstPtr& msg)
{
}

void DomEstimatorMono::load_object_param()
{
...
void DomEstimatorMono::load_objects()
{
    ...
    //         double dom = static_cast<double>(std::stod(strvec[3]));
    ...
    //         database_->add_init_object(id,name,time,dom,x,y);
    //     }
...
void DomEstimatorMono::setup_object_texts()
{
    // // is database empty?
    // if(database_->empty()){
...
void DomEstimatorMono::setup_time_text()
{
    // default
...
void DomEstimatorMono::save_objects(std::string record_file)
{
...
void DomEstimatorMono::visualize_object()
{
...
void DomEstimatorMono::publish_object_texts()
{
...
void DomEstimatorMono::publish_object()
{
    ...
    for(auto it = database_->begin(); it != database_->end(); it++){
        ...
        // simple object data
        multi_localizer_msgs::SimpleObjectData data;
...
void DomEstimatorMono::publish_msg()
{
    visualize_object();
...
geometry_msgs::Pose DomEstimatorMono::get_pose_msg(double x,double y)
{
...
std_msgs::ColorRGBA DomEstimatorMono::get_color_msg(double r,double g,double b,double a)
{
...
std_msgs::ColorRGBA DomEstimatorMono::get_background_color()
{
    // background color is white
    return get_color_msg(1.0,1.0,1.0,1.0);
}

std::vector<std::string> DomEstimatorMono::split(std::string& input,char delimiter)
{
...
std::string DomEstimatorMono::get_date()
{
...
double DomEstimatorMono::get_time() { return (ros::Time::now() - start_time_).toSec(); }

void DomEstimatorMono::process()
{
